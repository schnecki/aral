...
params :: Parameters
params = Parameters 0.1 3 0.15 0.05 0.05 0.01 1.0 1.0 0.1 0.75 0.2
...
decay t _ _ p@(Parameters minRho initRho alp bet del ga eps exp rand zeta xi)
  | t `mod` 200 == 0 = Parameters minRho initRho (f $ slower * alp) (f $ slow * bet) (f $ slow * del) ga (max 0.1 $ slower * eps) (f $ slower * exp) rand zeta xi -- (1 - slower * (1-frc)) mRho
  | otherwise = p
