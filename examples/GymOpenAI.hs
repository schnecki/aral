...
main = do
  Py.initialize
  Py.setArgv "gym-py" []
  gym <- python $ Py.importModule "gym"
  pyName <- Py.toUnicode "CartPole-v0"
  env <- python $ Py.callMethodArgs gym "make" [Py.toObject pyName]
  observation <- python $ Py.callMethodArgs env "reset" []

  forM_ ([1..100] :: [Int]) $ \x -> do
    void $ python $ Py.callMethodArgs env "render" []
    void $ python $ Py.print observation stdout
    actionSpace <- Py.getAttribute env =<< Py.toUnicode "action_space"
    sample <- python $ Py.callMethodArgs actionSpace "sample" []
    void $ python $ Py.callMethodArgs env "step" [Py.toObject sample]
    putStrLn $ "Step" ++ show x
    jsonActionSpace <- python $ Py.callMethodArgs actionSpace "__repr__" []
    textOrNone json >>= printActionSpace
