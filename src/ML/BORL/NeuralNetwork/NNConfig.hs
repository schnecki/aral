...
instance NFData NNConfig where
  rnf (NNConfig rep tr lp dec pp sc stab stabDec up upDec mse param) =
    rnf rep `seq` rnf tr `seq` rnf lp `seq` rnf dec `seq` rnf pp `seq` rnf sc `seq` rnf stab `seq` rnf stabDec `seq` rnf up `seq` rnf upDec `seq` rnf mse `seq` rnf param
