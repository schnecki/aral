...
instance NFData NNConfig where
  rnf (NNConfig rep tr lp dec pp sc stab stabDec up mse) =
    rnf rep `seq` rnf tr `seq` rnf lp `seq` rnf dec `seq` rnf pp `seq` rnf sc `seq` rnf stab `seq` rnf stabDec `seq` rnf up `seq` rnf mse
