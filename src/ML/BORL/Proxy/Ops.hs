...
mkNNList :: (Eq k) => BORL k -> Bool -> Proxy k -> T.MonadBorl [(k, ([(ActionIndex, Double)], [(ActionIndex, Double)]))]
mkNNList borl scaled pr =
  ...
  where
    ...
    lookupTable scale st | scale = val -- values are being unscaled, thus let table value be unscaled
                         | otherwise = map (scaleValue (getMinMaxVal pr)) val

      where val = map snd $ M.toList $ M.filterWithKey (\(x,_) _ -> x == st) (_proxyNNStartup pr)
