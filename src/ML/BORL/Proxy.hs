...
trainMSE mPeriod dataset lp px@(Grenade _ netW tab tp config nrActs)
  ...
  where
    ...
    getValue k = head $ snd $ fromLastShapes netW $ runNetwork netW ((toHeadShapes netW . (config ^. toNetInp) . fst) k)
...
-- | Retrieve a value from a neural network proxy. For other proxies an error is thrown. The returned value is up-scaled
-- to the original interval before returned.
lookupActionsNeuralNetwork :: LookupType -> (k, ActionIndex) -> Proxy k -> T.MonadBorl [Double]
...
mkNNList :: Bool -> Proxy k -> T.MonadBorl [((k, ActionIndex), (Double, Double))]
mkNNList scaled pr =
  concat <$>
  mapM
    (\st ->
       mapM
         (\actIdx -> do
            t <-
              if scaled
                then lookupNeuralNetwork Target (st, actIdx) pr
                else lookupNeuralNetworkUnscaled Target (st, actIdx) pr
            w <-
              if scaled
                then lookupNeuralNetwork Worker (st, actIdx) pr
                else lookupNeuralNetworkUnscaled Worker (st, actIdx) pr
            return ((st, actIdx), (t, w)))
         actIdxs)
