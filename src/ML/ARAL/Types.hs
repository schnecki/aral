...
#ifdef DEBUG
multiAgentValue xs | V.null xs = error "empty input list in singleAgentValues in ML.ARAL.Types"
#endif
...
#ifdef DEBUG
multiAgentValues xs
  | VB.null xs = error "empty input list in singleAgentValues in ML.ARAL.Types"
  | any (/= l) ls = error $ "length in input list in singleAgentValues in ML.ARAL.Types do not match: " ++ show (l : ls)
  where
