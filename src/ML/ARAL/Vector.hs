...
unsafeMemCopyVectorFromTo :: V.Vector Double -> V.Vector Double -> V.Vector Double
unsafeMemCopyVectorFromTo from to = unsafePerformIO (memCopyVectorFromTo from to)
{-# NOINLINE unsafeMemCopyVectorFromTo #-}

-- | Memory copy a vector from one to the other.
memCopyVectorFromTo :: V.Vector Double -> V.Vector Double -> IO (V.Vector Double)
...
unsafeMemZero :: V.Vector Double -> V.Vector Double
unsafeMemZero = unsafePerformIO . memZero


-- | Write zero to all elements in a vector.
memZero :: V.Vector Double -> IO (V.Vector Double)
memZero vec = do
  V.unsafeWith vec $ \vecPtr' ->
    void $ memset vecPtr' 0 (fromIntegral $ sizeOf (0 :: Double) * V.length vec)
  return vec
{-# INLINE memZero #-}
...
-- | map on Vectors (code from HMatrix)
mapVectorInPlace :: (Double -> Double) -> V.Vector Double -> V.Vector Double
...
zipWithVector :: (Double -> Double -> Double) -> V.Vector Double -> V.Vector Double -> V.Vector Double
zipWithVector f u v =
  unsafePerformIO $ do
...
zipWithVectorInPlaceSnd :: (Double -> Double -> Double) -> V.Vector Double -> V.Vector Double -> V.Vector Double
...
-- | zipWith for Vectors (code from HMatrix)
sumVectors :: [V.Vector Double] -> V.Vector Double
sumVectors [] = V.empty
sumVectors vs =
  -- unsafePerformIO $ do
